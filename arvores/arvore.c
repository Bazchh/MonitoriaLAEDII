...
struct card construir(int p1, int p2, int p3, int p4) {
  struct card carta;
  carta.p1 = p1;
  carta.p2 = p2;
  carta.p3 = p3;
...
int main() {
  ...
  raiz = 0;
  struct card A = construir(2, 0, 0, 0);
  raiz = inserirNovoNo(raiz, A);
  struct card B = construir(2, 0, 1, 1);
  raiz = inserirNovoNo(raiz, B);
  struct card C = construir(2, 0, 2, 2);
  raiz = inserirNovoNo(raiz, C);
  mostrarArvore(raiz);
}
int func(struct card carta) {
  return carta.p1 + 3 * carta.p2 + 9 * carta.p3 + 27 * carta.p4;
}
...
struct arvore *inserirNovoNo(struct arvore *raiz, struct card dados) {
  ...
  // Verificando balanceamento do no atual para iniciar processo de
  // balanceamento
...
//     // Ao realizar o processo anterior, agora precisamos remover dados
//     // duplicados na arvore, neste caso, o sucessor do numero a ser removido
//     que
//     // foi encontrado
...
void mostrarArvore(struct arvore *raiz) {
  if (raiz != 0) {
    ...
    printf("Carta = (%d, %d, %d, %d)\n", raiz->dados.p1, raiz->dados.p2,
           raiz->dados.p3, raiz->dados.p4);
